...
test.describe("Unique Submission", () => {
    ...
    test("Verify unique submission behaviour", async ({ page, formBuilderPage, browser }) => {
        ...
        await test.step("Step 1: Navigate to settings and enable Unique Submission with cookie tracking", async () => {
            await formBuilderPage.enableUniqueSubmissionWithTracking('cookie');
        });

        await test.step("Step 2: Submit the form once from the same browser (first submission should succeed)", async () => {
            const publishedPage = await formBuilderPage.submitPublishedForm(email);
        });

        await test.step("Step 3: Open the form again in the same browser — should be blocked by cookie", async () => {
        ...
        await test.step("Step 4: Submit the form from a new browser context — should succeed (different cookie)", async () => {
            await formBuilderPage.submitFormInNewContext(browser, faker.internet.email());
        });

        await test.step("Step 5: Switch to No check — disable unique submission tracking", async () => {
            await page.goto(formEditorUrl);
            await page.waitForLoadState('domcontentloaded');
            await formBuilderPage.enableUniqueSubmissionWithTracking('noCheck');
        });

        await test.step("Step 6: Same browser can now submit multiple times without restriction", async () => {
