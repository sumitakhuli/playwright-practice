...
test.describe("Conditional logic", () => {
    ...
    test("Conditional Logic for form", async ({ page, formBuilderPage }) => {
        ...
        await test.step("Step 1: Add option and email fields to the form", async () => {
        ...
        await test.step("Step 2: Moving to the conditional logic tab", async () => {
            await formBuilderPage.navigateToSettings();
            await formBuilderPage.addCondition();
            formEditorUrl = page.url();
        });

        await test.step("Step 3: Adding conditions", async () => {
            await formBuilderPage.addConditionalLogic();
            formEditorUrl = page.url();
        });

        await test.step("Step 4: Publishing the form", async () => {
            await formBuilderPage.publishForm();
            const publishedPage = await formBuilderPage.getPublishedPage();

            await test.step("Step 4.1: Verifying the conditional logic", async () => {
                await publishedPage.verifyConditionalLogic();
            });

            await test.step("Step 4.2: Close the form", async () => {
                await publishedPage.close();
            });
        });

        await test.step("Step 5: Change conditonal Logic", async () => {
            await formBuilderPage.changeConditionalLogic();
            const publishedPage = await formBuilderPage.getPublishedPage();

            await test.step("Step 5.1: Verifying the new conditional logic", async () => {
